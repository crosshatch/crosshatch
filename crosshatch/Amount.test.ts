...
describe(import.meta.url, () => {
  it.effect(
    ...
    Effect.fn(function* () {
      ...
      assertAmount(yield* Amount.from(" 1.5 "), "1.5")
      assertAmount(yield* Amount.from(".5"), "0.5")
      assertAmount(yield* Amount.from("1."), "1")
      assertAmount(yield* Amount.from("0.000001"), "0.000001")
      assertAmount(yield* Amount.from("0"), "0")
      assertAmount(yield* Amount.from("+1"), "1")
    }),
  )
  ...
  it.effect(
    ...
    Effect.fn(function* () {
      ...
      assert.strictEqual(yield* Amount.toAtomic(yield* Amount.from("1e5"), { decimals: 6 }), "100000000000")
      assert.strictEqual(yield* Amount.toAtomic(yield* Amount.from(1), { decimals: 0 }), "1")
      assert.strictEqual(yield* Amount.toAtomic(yield* Amount.from("1.1"), { decimals: 0 }), "2")
      assert.strictEqual(yield* Amount.toAtomic(yield* Amount.from("1.1"), { decimals: 0, rounding: "floor" }), "1")
  ...
  it.effect(
    ...
    Effect.fn(function* () {
      ...
      assert.isTrue(S.isSchemaError(yield* schemaError(S.decodeEffect(Amount.AmountFromString)(""))))
      assert.isTrue(S.isSchemaError(yield* schemaError(S.decodeEffect(Amount.AmountFromString)("   "))))
