...
describe("solana client", { concurrent: false }, () => {
  it.effect("deduplicates concurrent metadata and genesis lookups", () =>
    Effect.gen(function* () {
      ...
      expect(values).toStrictEqual(Array.from({ length: 10 }, () => ({ decimals: 6, programAddress: legacy })));
  ...
  it.effect("decodes an extended Token-2022 mint", () =>
    Effect.gen(function* () {
      ...
      const client = yield* SolanaClient;
      expect(yield* client.getMintMetadata(mint, network)).toStrictEqual({ decimals: 6, programAddress: token2022 });
    }).pipe(Effect.provide(layer("http://localhost:8899"))),
  );
  ...
  it.effect("retries failed metadata and genesis requests", () =>
    Effect.gen(function* () {
      ...
      expect((yield* Effect.flip(client.getMintMetadata(mint, network)))._tag).toBe("ProgramNotFoundError");
      yield* client.getMintMetadata(mint, network);
      expect(mocks.genesis).toHaveBeenCalledTimes(2);
  ...
  it.effect("rejects missing, unsupported, malformed, and uninitialized accounts", () =>
    Effect.gen(function* () {
      ...
      for (const [value, tag] of [
        [{ exists: false }, "ProgramNotFoundError"],
        [account(mint), "UnsupportedProgramError"],
        [account(legacy, 165), "GetMintMetadataError"],
        [account(token2022, 12), "GetMintMetadataError"],
        [wrongType, "GetMintMetadataError"],
        [uninitialized, "InvalidMintAccountError"],
        [{ ...account(), executable: true }, "InvalidMintAccountError"],
      ] as const) {
        mocks.account.mockResolvedValueOnce(value);
        expect((yield* Effect.flip(client.getMintMetadata(mint, network)))._tag).toBe(tag);
      }
    }).pipe(Effect.provide(layer("http://localhost:8899"))),
  );
