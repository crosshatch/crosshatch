...
const capturingFetch = (...bodies: Array<unknown>) => {
...
const chatCompletion = (message: unknown, finishReason: string = "stop") => ({
  id: "chatcmpl-test",
...
describe(import.meta.url, () => {
  ...
  it("streams text deltas through the openai-chat adapter when enabled", async () => {
    const requests: Array<any> = []
    const sseBody = [
      `data: {"id":"c1","model":"test-model","choices":[{"index":0,"delta":{"role":"assistant","content":"Hel"}}]}`,
      ``,
      `data: {"id":"c1","choices":[{"index":0,"delta":{"content":"lo"}}]}`,
      ``,
      `data: {"id":"c1","choices":[{"index":0,"delta":{},"finish_reason":"stop"}],"usage":{"prompt_tokens":1,"completion_tokens":2}}`,
      ``,
      `data: [DONE]`,
      ``,
    ].join("\n")
    const sseFetch: typeof globalThis.fetch = async (input, init) => {
      const request = input instanceof Request ? input : new Request(input, init)
      requests.push(await request.clone().json())
    ...
    const layer = OpenAiChatCompletions.layer({
      ...
      maxTokens: 16,
      streaming: true,
    }).pipe(Layer.provide(Layer.succeed(FetchHttpClient.Fetch, sseFetch)))
